...
def train_epoch(model, data, num_workers, optimizer, loss_fn, device, max_iter):
    ...
    for iter_idx, data_item in enumerate(tqdm(train_loader)):
        ...
        y_pred = torch.sigmoid(model(coded_seq))
...
if __name__ == "__main__":
    ...
    # Train SpliceFinder from scratch.
    ...
    log_file = os.path.join(ROOT_DIR, 'comparisons', 'results', 'SpliceFinder', f'log_seed-{cmd_args.random_seed}.txt')
    model_save_path = os.path.join(ROOT_DIR, 'comparisons', 'results', 'SpliceFinder', f'model_seed-{cmd_args.random_seed}.pt')
    os.makedirs(os.path.dirname(model_save_path), exist_ok=True)

    log(f'[SpliceFinder] Training begins.', filepath=log_file)
