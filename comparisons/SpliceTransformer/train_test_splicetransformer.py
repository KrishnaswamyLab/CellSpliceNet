...
if __name__ == "__main__":
    cmd_parser = argparse.ArgumentParser(description="SpliceTransformer comparison baseline.")
    ...
    model = SpliceTransformer(in_channels=2, max_seq_len=COMPARISON_SEQ_LEN).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=cmd_args.learning_rate)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.99)
    loss_fn = torch.nn.MSELoss()

    log_file, model_save_path = comparison_run_paths("SpliceTransformer", cmd_args.data_tag, cmd_args.random_seed)

    log(f"[SpliceTransformer] Training begins (seq_len={COMPARISON_SEQ_LEN}).", filepath=str(log_file))
    run_step_training(
        model=model,
        data=data,
        device=device,
